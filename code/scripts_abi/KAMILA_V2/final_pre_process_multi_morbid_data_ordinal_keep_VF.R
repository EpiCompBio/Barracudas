...
deborah_data=read.csv("Data/CVD_diag_by_doc_236_csv",sep=",")
colnames(deborah_data)[1]="eid"

CVD_ICD10_codes=read.table("Data/CVD_icd10codes.txt")[,1]

deborah_data2=read.csv("Data/CVD_10035_236.csv")
...
#define waist/hip ratio and remove waist/hip circumference variables
merged_data$waist_hip_ratio <- merged_data$waist_circum/merged_data$hip_circum
merged_data$waist_circum <- NULL
merged_data$hip_circum <- NULL

#remove all height variables except height
merged_data$height_sitting <- NULL
merged_data$sitting_height <- NULL
merged_data$seated_box_height <- NULL

#remove body water mass
merged_data$whole_body_water_mass <- NULL

#remove asthma variable 
merged_data$dvt_asthma_copd_atopy <- NULL
...
#re-organize columns
merged_data=merged_data %>% dplyr::select(CAD,stroke,obese,diabetes,htn,
...
saveRDS(merged_data,"Data/KAMILA_V2/unscaled_full_data_ordinal_keep.rds")
saveRDS(multi_morbid,"Data/KAMILA_V2/unscaled_multi_morbid_ordinal_keep.rds")

##### Scale data #####

#scale numeric features
merged_data[,-c(1,9,binary_col_ids,cat_col_ids,ord_col_ids)] =
  as.data.frame(scale(merged_data[,-c(1,9,binary_col_ids,cat_col_ids,ord_col_ids)]))
...
saveRDS(merged_data,"Data/KAMILA_V2/full_data_ordinal_keep.rds")
saveRDS(multi_morbid,"Data/KAMILA_V2/multi_morbid_ordinal_keep.rds")
